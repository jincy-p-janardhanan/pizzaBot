...
const datastore = new Datastore({
  projectId: 'qwiklabs-gcp-01-a474ea6814bd'
});
...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  function order_pizza(agent) {
      ...
      var milk_type = agent.parameters.drink_composite.milk_type;
      const taskKey = datastore.key('order_item');
      const entity = {
        key: taskKey,
        data: {
      ...
      return datastore.save(entity).then(() => {
                console.log(`Saved ${entity.key.name}: ${entity.data.item_name}`);
                agent.add(`Your order for ${drink} has been placed!`);
  ...
  intentMap.set('order.pizza - yes', order_drink);
